...
Diese funktioniert ähnlich wie `ring`, kann aber auch
Aussagen zeigen, die leicht zu beweisen sind, auch Aussagen,
die nicht in allgemeinen Ringen gelten.

`simp` kann sowohl verwendet werden, um ein Ziel zu zeigen, als auch,
um als Zwischenschritt zu vereinfachen.
In letzterem Fall sollte `simp?` verwendet werden, damit
auch zukünftige Versionen von Lean an dieser Stelle
dieselbe Vereinfachung durchführen.

`simp` vereinfacht per Default das Ziel.
