...
Definiere `PythagoreanTriple'`, d.h.
...
example (G : Type*) [AbelianGroup' G] : AbelianGroup' (Triple G) where
  ...
  comm := by
    intro a b
    ext
    all_goals apply AbelianGroup'.comm
  assoc := by
    intro a b c
    ext
    all_goals apply AbelianGroup'.assoc
  zero := {
    x := AbelianGroup'.zero
    y := AbelianGroup'.zero
    z := AbelianGroup'.zero
  ...
  add_zero := by
    intro a
    ext
    all_goals apply AbelianGroup'.add_zero
  neg := fun a ↦ {
    x := AbelianGroup'.neg a.x
    y := AbelianGroup'.neg a.y
    z := AbelianGroup'.neg a.z
  }
  add_neg := by
    intro a
    ext
    all_goals apply AbelianGroup'.add_neg
